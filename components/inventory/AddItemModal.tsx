...
const getSupabaseErrorMessage = (error: unknown, fallback: string) => {
  ...
  if (fullError.includes('inventory_category') || fullError.includes('type') && fullError.includes('does not exist')) {
    return 'Database schema error: The "inventory_category" type constraint is blocking this operation. Please contact your administrator to run the supabase-fix-category-enum.sql migration to fix this issue.';
  }
